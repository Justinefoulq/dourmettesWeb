...
router.get('/location/:NumLoc', async (req,res) => {
	...
	const semdispo = await pool.query('SELECT S.NumSemaine FROM semaine S WHERE S.NumSemaine NOT IN (SELECT E.NumSemaine FROM effectue E, reservation R WHERE R.NumResa=E.NumResa AND R.ResaRefus!=1 AND E.NumLoc=?)',[NumLoc]);
